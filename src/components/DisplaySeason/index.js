...
class DisplaySeason extends Component {
  ...
  render() {
        ...
        if ( (this.props.store.app.seasonStatus) && (this.props.store.app.getChartData) ) {
            ...
            const getXaxisMin = () => {
                return Date.UTC(firstYearOfSeason,8,1)
            }
            ...
            const afterRender = (chart) => { chart.renderer.text(firstYearOfSeason+'-'+secondYearOfSeason+' Season', 325, 85).css({ color:"#000000", fontSize:"16px"}).add() };
            ...
            var chartConfig = {
                 ...
                 series: [{
                     ...
                     name: '50% Damage Temp', data: data[variety]['T50'].slice(0,idxFirstFcst),   color: '#FFD700', lineWidth: 2, marker: { enabled: true, radius: 3 } },{
                     name: '50% Damage Fcst', pointStart: firstFcstDate, data: data[variety]['T50'].slice(idxFirstFcst), dashStyle: 'dot', color: '#FFD700', lineWidth: 2, marker: { enabled: true, radius: 3 }, showInLegend: fcstInView() },{
